...
/** Implemented by a scripted class so `InsanityType` can reflect on it like a native `Class`. */
...
/** Implemented by a scripted enum so `InsanityType` can reflect on it like a native `Enum`. */
...
/** Implemented by a scripted enum value so `InsanityType` can reflect on it like a native `EnumValue`. */
