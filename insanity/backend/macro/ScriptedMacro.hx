	...
	public static macro function build():Array<Field> {
		...
		var newExpr = macro {
			...
			function setFields(t:insanity.backend.types.Scripted.InsanityScriptedClass, isSuper:Bool = false) {
				...
				if (isSuper)
					__interp.locals.set('super', {r: insanity.backend.Expr.Mirror.MSuper(superLocals, constructor ?? __constructSuper)});
			}
		...
		fields.push({
			...
			kind: FFun({
				args: [
					{name: 'base', type: macro :insanity.backend.types.Scripted.InsanityScriptedClass},
		...
		fields = fields.concat([
			...
			{ // TODO
				...
				kind: FFun({
					...
					expr: macro {
						if (insanity.backend.macro.ScriptedMacro.ignoreFields.contains(property))
							return null;
					...
					expr: macro {
						var fields = [
							for (f in Reflect.fields(this))
								if (!insanity.backend.macro.ScriptedMacro.ignoreFields.contains(f)) f
						];
						for (f in __vars.keys()) {
							if (!insanity.backend.macro.ScriptedMacro.ignoreFields.contains(f) && !fields.contains(f))
								fields.push(f);
						}
						return fields;
					},
