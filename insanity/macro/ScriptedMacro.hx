...
class ScriptedMacro {
	...
	static var _name:String = 'insanity.macro.ScriptedMacro';
	...
	public static macro function build():Array<Field> {
		...
		if (Context.defined('insanity_verbose'))
			Context.info('Preparing ${cls.name}', pos);

		cls.meta.add(':access', [macro insanity.Module], pos);
		cls.meta.add(':access', [macro insanity.runtime.Interp], pos);
		cls.meta.add(':access', [macro insanity.types.ScriptedClass], pos);
		...
		var newExpr = macro {
			...
			function setInstanceFields(i:Dynamic) {
				...
				var superLocals:Map<String, insanity.runtime.Variable> = __interp.duplicate(__interp.locals);

				for (field in instanceFields) {
					if (insanity.macro.ScriptedMacro.ignoreFields.contains(field))
						continue;

					if (!__interp.variables.exists(field))
						__interp.variables.set(field, insanity.runtime.Mirror.MProperty(this, field));
				...
				__interp.locals.set('super', {r: insanity.runtime.Mirror.MSuper(superLocals, __constructSuper)});
			}
			function setFields(t:insanity.types.ScriptedClass, isSuper:Bool = false) {
					...
					switch (field.kind) {
						...
						case KVar(v):
							if (instanceFields.contains(f)) {
								...
								var l:insanity.runtime.Variable = {
									r: null,
									access: field.access,
									get: v.get,
									set: v.set
				...
				if (isSuper)
					__interp.locals.set('super', {r: insanity.runtime.Mirror.MSuper(superLocals, constructor ?? __constructSuper)});
			}

			function setSuperFields(extending:Dynamic) {
				if (extending is insanity.types.ScriptedClass) {
					var extend:insanity.types.ScriptedClass = cast extending;
		...
		fields.push({
			...
			kind: FFun({
				args: [
					{name: 'base', type: macro :insanity.types.ScriptedClass},
		...
		fields = fields.concat([
			{
				pos: pos,
				name: '__base',
				kind: FVar(macro :insanity.types.ScriptedClass),
			},
			{
				pos: pos,
				name: '__safe',
			...
			},
			{
				pos: pos,
				name: '__vars',
				kind: FVar(macro :Map<String, insanity.runtime.Variable>),
			},
			{
				pos: pos,
				name: '__fields',
			...
			},
			{
				pos: pos,
				name: '__interp',
				kind: FVar(macro :insanity.runtime.Interp),
			},
			{
				pos: pos,
			...
			{ // TODO
				...
				kind: FFun({
					...
					expr: macro {
						if (insanity.macro.ScriptedMacro.ignoreFields.contains(property))
							return null;
					...
					expr: macro {
						var fields = [
							for (f in Reflect.fields(this))
								if (!insanity.macro.ScriptedMacro.ignoreFields.contains(f)) f
						];
						for (f in __vars.keys()) {
							if (!insanity.macro.ScriptedMacro.ignoreFields.contains(f) && !fields.contains(f))
								fields.push(f);
						}
						return fields;
					},
			...
			},
			{
				...
				kind: FFun({
					...
					ret: macro :insanity.types.ScriptedClass
				})
			},
			{
				pos: pos,
