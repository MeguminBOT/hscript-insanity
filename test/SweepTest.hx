...
class SweepTest {
	...
	public static function main():Void {
		...
		Sys.println('');
		Sys.println(refused + ' refused, ' + wrong + ' wrong');
	}

	static function run(name:String, body:String, want:String, extra:String):Void {
		...
		var mark:String = StringTools.startsWith(compiled, 'REFUSED') ? '  REFUSED'
			: (compiled == want && interp == want ? '  ok     ' : '  WRONG  ');

		Sys.println(mark + StringTools.rpad(name, ' ', 20) + 'interp=' + StringTools.rpad(interp, ' ', 10) + 'cppia=' + compiled);
	}
