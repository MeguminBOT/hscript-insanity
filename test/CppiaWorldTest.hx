...
';
	...
	public static function main():Void {
		...
		Sys.println('');
		Sys.println('----------------------------------------');
		Sys.println(failures == 0 ? 'every world agreed with the interpreter' : failures + ' worlds disagreed');
	...
	static function check(label:String, compile:Array<String>, expectSubstituted:Bool = false):Void {
		var want:String = run([]);
		substituted = false;
		var got:String = run(compile);

		if (want != got) {
			failures++;
			Sys.println('  FAIL ' + label + '   interpreted ' + want + ', compiled ' + got);
			return;
		}
		...
		Sys.println('  ok   ' + label + '   ' + got + (substituted ? '   (ran compiled)' : '   (ran interpreted)'));
	}
	 ...
	 */
	static function run(compile:Array<String>):String {
		try {
