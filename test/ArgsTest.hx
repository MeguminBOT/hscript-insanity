...
class ArgsTest {
	static var pass = 0; static var fail = 0;
	static function eq(name:String, got:Dynamic, want:Dynamic) {
		if (Std.string(got) == Std.string(want)) { pass++; trace("  ok   " + name + " = " + got); }
		else { fail++; trace("  FAIL " + name + " got " + got + " want " + want); }
	}
	static function ev(body:String):Dynamic {
		var s = new Script("result = (" + body + ");", "a"); s.start(); return s.variables.get("result");
	}
