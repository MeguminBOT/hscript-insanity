...
class DceProbe {
	...
	static function p(name:String, src:String, want:String):Void {
		...
		var got:String = Std.string(s.variables.get("res"));
		var ok:Bool = got == want;
		if (!ok)
			gaps++;

		Sys.println((ok ? "  ok    " : "  GAP   ") + StringTools.rpad(name, " ", 22) + got + (ok ? "" : "   (want " + want + ")"));
	}
	...
	public static function main():Void {
		...
		Sys.println("");
		Sys.println(gaps == 0 ? "all reachable" : gaps + " unreachable");
	}
}
