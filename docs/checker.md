...
a type inference engine, and every rule in it is a chance to **reject a valid script** — far worse
...
It reports a list rather than throwing on the first problem — the value of a pre-run check is seeing
everything at once — and is **opt-in**. A host that loads user scripts at runtime may prefer to run
